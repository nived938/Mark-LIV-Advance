...
# Additional locations that Mark-LIV is allowed to read/write.
# This is useful for projects stored on another drive, such as G:\Coding.
#
# Add or remove directories here depending on where you want JARVIS
# to be able to manage files.
...
def _is_safe_path(target: Path) -> bool:
    """
    Check whether a path is inside an allowed location.

    The original Mark-LIV implementation only allowed paths underneath
    Path.home(). That prevents normal Windows project folders on another
    drive from being written to.

    We explicitly allow G:/Coding as an additional workspace.
    """

    try:
        resolved = target.resolve()

        allowed_roots = _SAFE_ROOTS + _ALLOWED_ROOTS
