"""Local J.A.R.V.I.S. startup authentication.

Password verification is local and stores only a salted PBKDF2-HMAC hash.
The optional voice unlock uses spoken-passphrase transcription; it is a
convenience unlock, not biometric speaker identification.
"""
...
def verify_password(password: str) -> bool:
    data = _load()
    return _verify(str(password or ""), str(data.get("password_salt") or ""), str(data.get("password_hash") or ""))
...
def verify_voice_phrase(phrase: str) -> bool:
    ...
    if not normalized:
        return False
    return _verify(normalized, str(data.get("voice_salt") or ""), str(data.get("voice_hash") or ""))
